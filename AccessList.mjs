...
async function sendTransactionWithAccessList(privateKey) {
  ...
  const nodeUrl = "https://polygon.gateway.tenderly.co";
  ...
  //Ethereum: https://mainnet.infura.io/v3/your_infura_key (another rpc can be used)
  //Pulse: https://rpc.pulsechain.com (another rpc can be used)
  let priceURL = "https://polygon.gateway.tenderly.co";
  let method = "POST";
  const options = {
    method: method,
    url: priceURL,
  ...
  await axios.request(options).then(async (result) => {
    ...
    //Specify 'value' (the value sent), 'type' (transaction type), 'gasLimit' (gas limit), and 'accessList' (access list).cons
